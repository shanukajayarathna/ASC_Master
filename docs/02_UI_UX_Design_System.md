...
### Responsive design
One build serves PC, tablets and phones; tablets are a primary field device (the Valuation Centre explicitly ships a "tablet-friendly focus mode" for sale-floor use). The whole route surface is audited headlessly at 390px (phone), 820px/1180px (tablet portrait/landscape) and 1440px (PC) for horizontal page overflow — the invariant is **zero page-level horizontal scroll at every width**; wide content (lot tables, AG Grid, report tables) scrolls inside its own container instead. Layouts get there with intrinsic patterns (`auto-fit`/`auto-fill` minmax grids, flex-wrap on the topbar, `min()`-capped drawer/dialog widths, `fullScreen` dialogs under the `sm` breakpoint), not per-device page variants. Test any new screen at phone and tablet widths, not just desktop. See [19_Performance.md](19_Performance.md) for related on-device performance concerns.

### Touch & installed-app conventions
`globals.css`'s "Cross-device / touch conventions" block is the single home for device-conditional behaviour; everything in it changes UI only where the device genuinely differs, so desktop density stays as designed:

- **`.hover-reveal`** — the pattern for hover-gated affordances (launchpad pin/open-arrow). Revealed on `.group` hover, on keyboard focus-within, and *always visible* under `@media (hover: none)` — an invisible-but-tappable control on a touch screen is a trap, never ship one.
- **Touch targets** — under `@media (pointer: coarse)`, small MUI icon buttons get 10px padding (≈40px hit area) and the tile pin grows from 28px to 36px. Desktop keeps its compact sizes.
- **No sticky hover states** — `.lift-on-hover` disables its translate under `hover: none`, since touch browsers emulate `:hover` on tap and leave it stuck.
- **iOS input zoom** — form controls are forced to 16px font under `(max-width: 640px) and (pointer: coarse)`; anything smaller makes iOS Safari zoom the page into every focused field.
- **Safe areas** — the viewport uses `viewport-fit=cover` and the manifest `black-translucent`, so the installed app draws under notches/home indicators; `.app-topbar` (top inset) and `.app-main-safe` (side/bottom insets) pad the shell back out via `env(safe-area-inset-*)`. These two classes *replace* the shell's padding utilities — change paddings there, not in Topbar/Shell class lists.
- **Mobile viewport units** — full-height columns (the Assistant's `.chat-vh`) use `dvh` with a `vh` fallback; plain `vh` hides bottom UI under the collapsing phone URL bar.
