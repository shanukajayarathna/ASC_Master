...
import type { CategoryOutcome, MarketShareCompareRow, RankTabData, RawValue } from "@/lib/weeklyFactReport";
...
export async function buildMarketShareComparePdf(
  ...
  tables.forEach((table) => {
    ...
    const body: RowInput[] = table.rows.map((r, i) => {
      const pct = totalQty > 0 ? (r.qty / totalQty) * 100 : 0;
      // The top-3 yellow highlight only covers rank/broker/% here (unlike the plain PDF's
      // full-row highlight) — the qty/diff columns stay unhighlighted per the user's request.
      ...
      return [
        ...
        { content: roundHalfAwayFromZero(r.qty).toLocaleString("en-US") },
