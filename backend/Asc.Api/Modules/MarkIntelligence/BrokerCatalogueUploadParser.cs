...
/// no header row, two (JK, LCBL) don't even carry their own Broker/Year/SaleNo columns. None
/// carry the "RP" reprint flag /data/sales' enriched General Report has, so reprint status is
/// inferred the same way it was validated against real RP=Yes rows earlier: NettWeight not
/// matching Chests x ChestWeight. Column positions below are keyed to the real Sale 36/2026
/// sample files; a broker changing its export format silently breaks its own parser — there is
/// no schema to validate against up front, so a wildly-wrong column count is the only signal
/// (rows with too few columns are simply skipped).
/// </summary>
public static class BrokerCatalogueUploadParser
{
    ...
    private static Lot? BuildLot(string broker, string mfCode, string sellingMark, string grade,
    ...
    public static List<Lot> ParseAeb(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            ...
            var lot = BuildLot(BrokerCode.Aeb, row[4], row[5], row[7], row[8], row[9], row[10]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseBc(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            ...
            var lot = BuildLot(BrokerCode.Bc, row[4], row[5], row[8], row[9], row[10], row[11]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseJk(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            var row = rows[r];
            if (row.Count < 12) continue;
            var lot = BuildLot(BrokerCode.Jk, row[1], row[2], row[6], row[7], row[8], row[11]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseLcbl(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            ...
            var lot = BuildLot(BrokerCode.Lcbl, row[1], row[2], row[4], row[5], row[7], row[10]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseMb(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            var row = rows[r];
            if (row.Count < 12) continue;
            var lot = BuildLot(BrokerCode.Mb, row[4], row[5], row[7], row[8], row[10], row[11]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseFw(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            var row = rows[r];
            if (row.Count < 12) continue;
            var lot = BuildLot(BrokerCode.Fw, row[4], row[5], row[7], row[8], row[10], row[11]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }

    public static List<Lot> ParseCtb(List<List<string>> rows)
    {
        ...
        for (var r = FirstDataRow; r < rows.Count; r++)
        {
            ...
            var lot = BuildLot(BrokerCode.Ctb, row[3], row[4], row[6], row[7], row[9], row[12]);
            if (lot is not null) lots.Add(lot);
        }
        return lots;
    }
    ...
    public static List<Lot> ParseAsc(CatalogueImportService importer, List<List<string>> rows)
    {
    ...
    public static List<Lot> Parse(string brokerCode, CatalogueImportService importer, List<List<string>> rows) => brokerCode switch
    {
        BrokerCode.Asc => ParseAsc(importer, rows),
        BrokerCode.Aeb => ParseAeb(rows),
        BrokerCode.Bc => ParseBc(rows),
        BrokerCode.Jk => ParseJk(rows),
        BrokerCode.Lcbl => ParseLcbl(rows),
        BrokerCode.Mb => ParseMb(rows),
        BrokerCode.Fw => ParseFw(rows),
        BrokerCode.Ctb => ParseCtb(rows),
