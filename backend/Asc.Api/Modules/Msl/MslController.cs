...
/// <summary>
/// Import, status and archive-maintenance endpoints over the MSL data (every auction lot
/// 2013–present plus the yearly private-sale files and the Tea Board monthly averages).
/// The sale-comparison endpoint joins MSL rows against the weekly sale Excel catalogues
/// (ICatalogueSource) on (broker, lot no) — the verified 99.7% join — surfacing
/// post-auction corrections. The MSL data itself is also read directly by
/// MslAnalyticsController and MslFilteredAnalyticsController (Analysis screen, Category
/// Analysis, weekly-fact comparisons) — those are separate controllers, not this one.
/// </summary>
[ApiController]
[Route("api/v1/msl")]
[Authorize]
